...
data Derivation : Judgment → Set where
  ...
  -- Conversion rules
  Conv : {Γ : Ctx n} {u : TmExpr n} {A B : TyExpr n} → Derivation (Γ ⊢ A) → Derivation (Γ ⊢ B) 
    → Derivation (Γ ⊢ u :> A) → Derivation (Γ ⊢ A == B) → Derivation (Γ ⊢ coerc A B u :> B)
  ConvEq : {Γ : Ctx n} {u u' : TmExpr n} {A B : TyExpr n} → Derivation (Γ ⊢ A)
    → Derivation (Γ ⊢ u == u' :> A) → Derivation (Γ ⊢ A == B) → Derivation (Γ ⊢ coerc A B u == coerc A B u' :> B)
  CoercRefl : {Γ : Ctx n} {u : TmExpr n} {A : TyExpr n} → Derivation (Γ ⊢ u :> A)
    → Derivation (Γ ⊢ coerc A A u == u :> A)
  CoercRefl! : {Γ : Ctx n} {u : TmExpr n} {A : TyExpr n} → Derivation (Γ ⊢ u :> A)
    → Derivation (Γ ⊢ u == coerc A A u :> A)
  CoercTrans : {Γ : Ctx n} {u : TmExpr n} {A B C : TyExpr n} → Derivation (Γ ⊢ A) → Derivation (Γ ⊢ B) → Derivation (Γ ⊢ C) → Derivation (Γ ⊢ u :> A) → Derivation (Γ ⊢ A == B) → Derivation (Γ ⊢ B == C) → Derivation (Γ ⊢ A == C) → Derivation (Γ ⊢ coerc B C (coerc A B u) == coerc A C u :> C)
  -- Rules for UU
  UU : {i : ℕ} {Γ : Ctx n}
    → Derivation (Γ ⊢ uu i)
  UUCong :  {i : ℕ} {Γ : Ctx n}
...
Γ ⊢ ◇ == ◇ ∷> ◇ = metaUnit
Γ ⊢ (δ , u) == (δ' , u') ∷> (Δ , A) = (Γ ⊢ δ == δ' ∷> Δ) × Derivation (Γ ⊢ u == coerc (A [ δ' ]Ty) (A [ δ ]Ty) u' :> A [ δ ]Ty)
...
-- WeakMor+coerc : {Γ : Ctx n} {Δ : Ctx m} {A : TyExpr m} {B : TyExpr n} {δ δ' : Mor n m} → Derivation (Δ ⊢ A) → Γ ⊢ δ ∷> Δ → Derivation (Γ ⊢ A [ δ ]Ty == B) → (Γ , A [ δ ]Ty) ⊢ weakenMor+coerc δ B (A [ δ ]Ty) ∷> (Δ , A)
-- WeakMor+coerc dA dδ dB= = WeakMor dδ , congTmTy (weaken[]Ty _ _ _) (Conv {!!} {!!} {!!} {!!})

{- I do not think that this is even valid, unless δ ≡ δ' -}
-- WeakMor+Eq : {Γ : Ctx n} {Δ : Ctx m} {A : TyExpr m} {δ δ' : Mor n m} → Derivation (Δ ⊢ A) → Γ ⊢ δ ∷> Δ → Γ ⊢ δ == δ' ∷> Δ → (Γ , A [ δ ]Ty) ⊢ weakenMor+coerc δ (A [ δ' ]Ty) (A [ δ ]Ty) == weakenMor+ δ' ∷> (Δ , A)
...
-- Conv (WeakTy dA) (WeakTm du) (WeakTyEq dA=) 
...
-- ConvEq (WeakTy dA) (WeakTmEq du=) (WeakTyEq dA=)
...
-- -- Conv (SubstTy dB (idMorDerivation dΓ , congTm! ([idMor]Ty _) refl (Pr1 dA dB (Pair dA dB da db)))) (Pr2 dA dB (Pair dA dB da db)) (SubstTyMorEq dB (idMorDerivation dΓ , congTm! ([idMor]Ty _) refl (Pr1 dA dB (Pair dA dB da db))) (MorRefl (idMorDerivation dΓ) , congTmEqTy! ([idMor]Ty _) (BetaSig1 dA dB da db)))
...
-- -- Conv (SubstTy (TyEqTy2 (dΓ , Empty) dA=) (idMorDerivation dΓ , TmEqTm2 dΓ du=)) (Emptyelim (TyEqTy2 (dΓ , Empty) dA=) (TmEqTm2 dΓ du=)) (SubstTyFullEq (TyEqTy1 (dΓ , Empty) dA=) (idMorDerivation dΓ , TmEqTm2 dΓ du=) (TySymm dA=) (MorRefl (idMorDerivation dΓ) , TmSymm du=))
...
getTy-Der {Γ = ◇} {app A₁ B u u₁} {.(B [ ◇ , u₁ ]Ty)} (App dj dj₁ dj₂ dj₃) dΓ = SubstTy dj₁ (ttR , congTmTy! ([idMor]Ty A₁) dj₃)
getTy-Der {Γ = Γ , A} {app A₁ B u u₁} {.(B [ (weakenMor' last (idMor _) , var last) , u₁ ]Ty)} (App dj dj₁ dj₂ dj₃) dΓ = SubstTy dj₁ ((idMorDerivation dΓ) , congTmTy! ([idMor]Ty A₁) dj₃)
...
-- helper : {Γ : Ctx n} {A B : TyExpr n} {u : TmExpr n} → Derivation (Γ ⊢ u :> B) → ΣSS (TyExpr n) (λ A → ΣS (TmExpr n) (λ u → Derivation (Γ ⊢ u :> A)))
-- helper du = {!!}
