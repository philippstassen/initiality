...
data Derivation : Judgment → Set where
  ...
  CoercTrans : {Γ : Ctx n} {u : TmExpr n} {A B C : TyExpr n} → Derivation (Γ ⊢ A) → Derivation (Γ ⊢ u :> A) → Derivation (Γ ⊢ A == B) → Derivation (Γ ⊢ B == C) → Derivation (Γ ⊢ A == C) → Derivation (Γ ⊢ coerc B C (coerc A B u) == coerc A C u :> C)
  -- Rules for UU
  UU : {i : ℕ} {Γ : Ctx n}
    → Derivation (Γ ⊢ uu i)
  UUCong :  {i : ℕ} {Γ : Ctx n}
...
-- WeakMor+Eq dA dδ dδ= = (WeakMorEq dδ= , {!(VarLast (SubstTy dA dδ))!} )
...
-- SubstTm {u = starU} STARU dδ = STARU
...
-- SubstTmEq STARUCong dδ = STARUCong
...
-- SubstTmMorEq STARU dδ dδ= = STARUCong
...
-- ConvTmEq STARUCong dΓ= = STARUCong
...
-- TmEqTm1 dΓ (BetaUnit dA ddtt) = Unitelim dA ddtt STARU
