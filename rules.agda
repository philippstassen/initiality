...
data Derivation : Judgment → Set where
  ...
  Conv : {Γ : Ctx n} {u : TmExpr n} {A B : TyExpr n} → Derivation (Γ ⊢ A)
    → Derivation (Γ ⊢ B) → Derivation (Γ ⊢ u :> A) → Derivation (Γ ⊢ A == B) → Derivation (Γ ⊢ u :> B)
  ...
  UU : {i : ℕ} {Γ : Ctx n}
    → Derivation (Γ ⊢ uu i)
  UUCong :  {i : ℕ} {Γ : Ctx n}
    → Derivation (Γ ⊢ uu i == uu i)

--   -- Rules for uu
--   UUUU : {i : ℕ} {Γ : Ctx n}
--     → Derivation (Γ ⊢ uu i :> uu (suc i))
--   UUUUCong : {i : ℕ} {Γ : Ctx n}
--     → Derivation (Γ ⊢ uu i == uu i :> uu (suc i))
--   ElUU= : {i : ℕ} {Γ : Ctx n}
--     → Derivation (Γ ⊢ el (suc i) (uu i) == uu i)

  -- Rules for El
  El : {i : ℕ} {Γ : Ctx n} {v : TmExpr n}
    → Derivation (Γ ⊢ v :> uu i) → Derivation (Γ ⊢ el i v)
  ElCong : {i : ℕ} {Γ : Ctx n} {v v' : TmExpr n}
    → Derivation (Γ ⊢ v == v' :> uu i) → Derivation (Γ ⊢ el i v == el i v')
...
--     → Derivation (Γ ⊢ a :> uu i) → Derivation ((Γ , el i a) ⊢ b :> uu i) → Derivation (Γ ⊢ pi i a b :> uu i)
--   PiUUCong : {i : ℕ} {Γ : Ctx n} {a a' : TmExpr n} {b b' : TmExpr (suc n)}
--     → Derivation (Γ ⊢ a :> uu i) → Derivation (Γ ⊢ a == a' :> uu i) → Derivation ((Γ , el i a) ⊢ b == b' :> uu i) → Derivation (Γ ⊢ pi i a b == pi i a' b' :> uu i)
--   ElPi= : {i : ℕ} {Γ : Ctx n} {a : TmExpr n} {b : TmExpr (suc n)}
--     → Derivation (Γ ⊢ a :> uu i) → Derivation ((Γ , el i a) ⊢ b :> uu i) → Derivation (Γ ⊢ el i (pi i a b) == pi (el i a) (el i b))
...
--     → Derivation (Γ ⊢ a :> uu i) → Derivation ((Γ , el i a) ⊢ b :> uu i) → Derivation (Γ ⊢ sig i a b :> uu i)
--   SigUUCong : {i : ℕ} {Γ : Ctx n} {a a' : TmExpr n} {b b' : TmExpr (suc n)}
--     → Derivation (Γ ⊢ a :> uu i) → Derivation (Γ ⊢ a == a' :> uu i) → Derivation ((Γ , el i a) ⊢ b == b' :> uu i) → Derivation (Γ ⊢ sig i a b == sig i a' b' :> uu i)
--   ElSig= : {i : ℕ} {Γ : Ctx n} {a : TmExpr n} {b : TmExpr (suc n)}
--     → Derivation (Γ ⊢ a :> uu i) → Derivation ((Γ , el i a) ⊢ b :> uu i) → Derivation (Γ ⊢ el i (sig i a b) == sig (el i a) (el i b))
...
--   -- Rules for empty
--   EmptyUU : {i : ℕ} {Γ : Ctx n}
--      → Derivation (Γ ⊢ empty i :> uu i)
--   EmptyUUCong : {i : ℕ} {Γ : Ctx n}
--      → Derivation (Γ ⊢ empty i == empty i :> uu i)
--   ElEmpty= : {i : ℕ} {Γ : Ctx n}
--      → Derivation (Γ ⊢ el i (empty i) == empty)
...
--   -- Rules for unit
--   UnitUU : {i : ℕ} {Γ : Ctx n}
--      → Derivation (Γ ⊢ unit i :> uu i)
--   UnitUUCong : {i : ℕ} {Γ : Ctx n}
--      → Derivation (Γ ⊢ unit i == unit i :> uu i)
--   ElUnit= : {i : ℕ} {Γ : Ctx n}
--      → Derivation (Γ ⊢ el i (unit i) == unit)
...
--   -- Rules for nat
--   NatUU : {i : ℕ} {Γ : Ctx n}
--     → Derivation (Γ ⊢ nat i :> uu i)
--   NatUUCong : {i : ℕ} {Γ : Ctx n}
--     → Derivation (Γ ⊢ nat i == nat i :> uu i)
--   ElNat= : {i : ℕ} {Γ : Ctx n}
--     → Derivation (Γ ⊢ el i (nat i) == nat)
...
--     → Derivation (Γ ⊢ a :> uu i) → Derivation (Γ ⊢ u :> el i a) → Derivation (Γ ⊢ v :> el i a) → Derivation (Γ ⊢ id i a u v :> uu i)
--   IdUUCong : {i : ℕ} {Γ : Ctx n} {a a' : TmExpr n} {u u' v v' : TmExpr n}
--     → Derivation (Γ ⊢ a == a' :> uu i) → Derivation (Γ ⊢ u == u' :> el i a) → Derivation (Γ ⊢ v == v' :> el i a) → Derivation (Γ ⊢ id i a u v == id i a' u' v' :> uu i)
--   ElId= : {i : ℕ} {Γ : Ctx n} {a : TmExpr n} {u v : TmExpr n}
--     → Derivation (Γ ⊢ a :> uu i) → Derivation (Γ ⊢ u :> el i a) → Derivation (Γ ⊢ v :> el i a) → Derivation (Γ ⊢ el i (id i a u v) == id (el i a) u v)
...
⊢_ : Ctx n → Set
⊢ ◇ = metaUnit
⊢ (Γ , A) = (⊢ Γ) × Derivation (Γ ⊢ A)

⊢_==_ : Ctx n → Ctx n → Set
⊢ ◇ == ◇ = metaUnit
⊢ (Γ , A) == (Γ' , A') = (⊢ Γ == Γ') × Derivation (Γ ⊢ A) × Derivation (Γ' ⊢ A') × Derivation (Γ ⊢ A == A') × Derivation (Γ' ⊢ A == A')

_⊢_∷>_ : (Γ : Ctx n) → Mor n m → Ctx m → Set
Γ ⊢ ◇ ∷> ◇ = metaUnit
Γ ⊢ (δ , u) ∷> (Δ , A) = (Γ ⊢ δ ∷> Δ) × Derivation (Γ ⊢ u :> A [ δ ]Ty) 

_⊢_==_∷>_ : (Γ : Ctx n) → Mor n m → Mor n m → Ctx m → Set
Γ ⊢ ◇ == ◇ ∷> ◇ = metaUnit
Γ ⊢ (δ , u) == (δ' , u') ∷> (Δ , A) = (Γ ⊢ δ == δ' ∷> Δ) × Derivation (Γ ⊢ u == u' :> A [ δ ]Ty)


{- Congruence with respect to the type in derivability of term expressions -}

congTy : {Γ : Ctx n} {A A' : TyExpr n} → A ≡ A' → Derivation (Γ ⊢ A) → Derivation (Γ ⊢ A')
congTy refl dA = dA

congTy! : {Γ : Ctx n} {A A' : TyExpr n} → A' ≡ A → Derivation (Γ ⊢ A) → Derivation (Γ ⊢ A')
congTy! refl dA = dA

congTyCtx :  {Γ Γ' : Ctx n} {A : TyExpr n} → Γ ≡ Γ' → Derivation (Γ ⊢ A) → Derivation (Γ' ⊢ A)
congTyCtx refl dA = dA

congTyCtxEq : {Γ Γ' : Ctx n} {A B : TyExpr n} → Γ ≡ Γ' → Derivation (Γ ⊢ A == B) → Derivation (Γ' ⊢ A == B)
congTyCtxEq refl dA= = dA=

congTyEq : {Γ : Ctx n} {A A' B B' : TyExpr n} → A ≡ A' → B ≡ B' → Derivation (Γ ⊢ A == B) → Derivation (Γ ⊢ A' == B')
congTyEq refl refl dA= = dA=

congTyEq! : {Γ : Ctx n} {A A' B B' : TyExpr n} → A' ≡ A → B' ≡ B → Derivation (Γ ⊢ A == B) → Derivation (Γ ⊢ A' == B')
congTyEq! refl refl dA= = dA=

congTm : {Γ : Ctx n} {A A' : TyExpr n} {u u' : TmExpr n} → A ≡ A' → u ≡ u' → Derivation (Γ ⊢ u :> A) → Derivation (Γ ⊢ u' :> A')
congTm refl refl du = du

congTm! : {Γ : Ctx n} {A A' : TyExpr n} {u u' : TmExpr n} → A' ≡ A → u' ≡ u → Derivation (Γ ⊢ u :> A) → Derivation (Γ ⊢ u' :> A')
congTm! refl refl du = du

congTmTy : {Γ : Ctx n} {A A' : TyExpr n} {u : TmExpr n} → A ≡ A' → Derivation (Γ ⊢ u :> A) → Derivation (Γ ⊢ u :> A')
congTmTy refl du = du

congTmTy! : {Γ : Ctx n} {A A' : TyExpr n} {u : TmExpr n} → A' ≡ A → Derivation (Γ ⊢ u :> A) → Derivation (Γ ⊢ u :> A')
congTmTy! refl du = du

congTmEqTy : {Γ : Ctx n} {A A' : TyExpr n} {u u' : TmExpr n} → A ≡ A' → Derivation (Γ ⊢ u == u' :> A) → Derivation (Γ ⊢ u == u' :> A')
congTmEqTy refl du= = du=

congTmEqTy! : {Γ : Ctx n} {A A' : TyExpr n} {u u' : TmExpr n} → A' ≡ A → Derivation (Γ ⊢ u == u' :> A) → Derivation (Γ ⊢ u == u' :> A')
congTmEqTy! refl du= = du=

congTmEqTm : {Γ : Ctx n} {A : TyExpr n} {u u' v v' : TmExpr n} → u ≡ v → u' ≡ v' → Derivation (Γ ⊢ u == u' :> A) → Derivation (Γ ⊢ v == v' :> A)
congTmEqTm refl refl du= = du=

congTmEq : {Γ : Ctx n} {A A' : TyExpr n} {u u' v v' : TmExpr n} → u ≡ v → u' ≡ v' → A ≡ A' → Derivation (Γ ⊢ u == u' :> A) → Derivation (Γ ⊢ v == v' :> A')
congTmEq refl refl refl du= = du=

congTmEq! : {Γ : Ctx n} {A A' : TyExpr n} {u u' v v' : TmExpr n} → v ≡ u → v' ≡ u' → A' ≡ A → Derivation (Γ ⊢ u == u' :> A) → Derivation (Γ ⊢ v == v' :> A')
congTmEq! refl refl refl du= = du=
...
congTyRefl : {Γ : Ctx n} {A A' : TyExpr n} → Derivation (Γ ⊢ A) → A ≡ A' → Derivation (Γ ⊢ A == A')
congTyRefl dA refl = TyRefl dA

congTyRefl' : {Γ : Ctx n} {A A' : TyExpr n} → Derivation (Γ ⊢ A') → A ≡ A' → Derivation (Γ ⊢ A == A')
congTyRefl' dA refl = TyRefl dA

congTmRefl : {Γ : Ctx n} {A : TyExpr n} {u u' : TmExpr n} → Derivation (Γ ⊢ u :> A) → u ≡ u' → Derivation (Γ ⊢ u == u' :> A)
...
SubstTy : {Γ : Ctx n} {Δ : Ctx m} {A : TyExpr m} {δ : Mor n m}
       → Derivation (Δ ⊢ A) → Γ ⊢ δ ∷> Δ → Derivation (Γ ⊢ A [ δ ]Ty)
SubstTm : {Γ : Ctx n} {Δ : Ctx m} {u : TmExpr m} {A : TyExpr m} {δ : Mor n m}
       → Derivation (Δ ⊢ u :> A) → Γ ⊢ δ ∷> Δ → Derivation (Γ ⊢ u [ δ ]Tm :> A [ δ ]Ty)
SubstTyEq : {Γ : Ctx n} {Δ : Ctx m} {A A' : TyExpr m} {δ : Mor n m}
       → Derivation (Δ ⊢ A == A') → Γ ⊢ δ ∷> Δ → Derivation (Γ ⊢ A [ δ ]Ty == A' [ δ ]Ty)
SubstTmEq : {Γ : Ctx n} {Δ : Ctx m} {u u' : TmExpr m} {A : TyExpr m} {δ : Mor n m}
       → Derivation (Δ ⊢ u == u' :> A) → (Γ ⊢ δ ∷> Δ) → Derivation (Γ ⊢ u [ δ ]Tm == u' [ δ ]Tm :> A [ δ ]Ty)
SubstTyMorEq : {Γ : Ctx n} {Δ : Ctx m} {A : TyExpr m} {δ δ' : Mor n m} → Derivation (Δ ⊢ A) → (Γ ⊢ δ ∷> Δ)
       → (Γ ⊢ δ == δ' ∷> Δ) → Derivation (Γ ⊢ A [ δ ]Ty == A [ δ' ]Ty)
SubstTmMorEq : {Γ : Ctx n} {Δ : Ctx m} {u : TmExpr m} {A : TyExpr m} {δ δ' : Mor n m} →  Derivation (Δ ⊢ u :> A) → (Γ ⊢ δ ∷> Δ) 
       → (Γ ⊢ δ == δ' ∷> Δ) → Derivation (Γ ⊢ u [ δ ]Tm == u [ δ' ]Tm :> A [ δ ]Ty)
...
WeakMorEq {Δ = ◇} {δ = ◇} {◇} dδ = ttR
WeakMorEq {Δ = Δ , B} {δ = δ , u} {δ' , u'} (dδ , du) = (WeakMorEq dδ , congTmEqTy (weaken[]Ty _ _ _) (WeakTmEq du))

WeakMor+~ : {Γ : Ctx n} {Δ : Ctx m} (A : TyExpr m) {δ : Mor n m} → Derivation (Γ ⊢ A [ δ ]Ty) → Γ ⊢ δ ∷> Δ → (Γ , A [ δ ]Ty) ⊢ weakenMor+ δ ∷> (Δ , A)
WeakMor+~ A dAδ dδ = (WeakMor dδ , congTmTy (weaken[]Ty _ _ _) (VarLast dAδ))

WeakMor+ : {Γ : Ctx n} {Δ : Ctx m} {A : TyExpr m} {δ : Mor n m} → Derivation (Δ ⊢ A) → Γ ⊢ δ ∷> Δ → (Γ , A [ δ ]Ty) ⊢ weakenMor+ δ ∷> (Δ , A)
WeakMor+ dA dδ = WeakMor+~ _ (SubstTy dA dδ) dδ

WeakMor+Eq : {Γ : Ctx n} {Δ : Ctx m} {A : TyExpr m} {δ δ' : Mor n m} → Derivation (Δ ⊢ A) → Γ ⊢ δ ∷> Δ → Γ ⊢ δ == δ' ∷> Δ → (Γ , A [ δ ]Ty) ⊢ weakenMor+ δ == weakenMor+ δ' ∷> (Δ , A)
...
SubstTmMorEq (Conv dA dB du dA=) dδ dδ= = ConvEq (SubstTy dA dδ) (SubstTmMorEq du dδ dδ=) (SubstTyEq dA= dδ)
...
SubstTyFullEq : {Γ : Ctx n} {Δ : Ctx m} {A A' : TyExpr m} {δ δ' : Mor n m} → Derivation (Δ ⊢ A') → (Γ ⊢ δ ∷> Δ)
       → Derivation (Δ ⊢ A == A') → (Γ ⊢ δ == δ' ∷> Δ) → Derivation (Γ ⊢ A [ δ ]Ty == A' [ δ' ]Ty)
SubstTyFullEq dA' dδ dA= dδ= = TyTran (SubstTy dA' dδ) (SubstTyEq dA= dδ) (SubstTyMorEq dA' dδ dδ=)

SubstTmFullEq : {Γ : Ctx n} {Δ : Ctx m} {u u' : TmExpr m} {A : TyExpr m} {δ δ' : Mor n m} →  Derivation (Δ ⊢ u' :> A) → (Γ ⊢ δ ∷> Δ) 
       → Derivation (Δ ⊢ u == u' :> A) → (Γ ⊢ δ == δ' ∷> Δ) → Derivation (Γ ⊢ u [ δ ]Tm == u' [ δ' ]Tm :> A [ δ ]Ty)
...
idMorDerivation : {Γ : Ctx n} →  ⊢ Γ → (Γ ⊢ idMor n ∷> Γ)
idMorDerivation {Γ = ◇} ttR = ttR
idMorDerivation {Γ = Γ , A} (dΓ , dA) = (WeakMor (idMorDerivation dΓ) , congTm (! ([idMor]Ty _) ∙ substTy-weakenTy') refl (VarLast dA))


{- Conversion rules for types and terms are admissible -}

-- ConvTy : {Γ Δ : Ctx n} {A : TyExpr n} → Derivation (Γ ⊢ A) → (⊢ Γ == Δ) → Derivation (Δ ⊢ A)
-- ConvTyEq : {Γ Δ : Ctx n} {A B : TyExpr n} → Derivation (Γ ⊢ A == B) → (⊢ Γ == Δ) → Derivation (Δ ⊢ A == B)
-- ConvTm : {Γ Δ : Ctx n} {u : TmExpr n} {A : TyExpr n} → Derivation (Γ ⊢ u :> A) → (⊢ Γ == Δ) → Derivation (Δ ⊢ u :> A)
-- ConvTmEq : {Γ Δ : Ctx n} {A : TyExpr n} {u v : TmExpr n} → Derivation (Γ ⊢ u == v :> A) → (⊢ Γ == Δ) → Derivation (Δ ⊢ u == v :> A)
-- 
-- {- added assumption Derivation (Γ ⊢ A'). needed due to changed Conv rule -}
-- ConvTm2' : {Γ Δ : Ctx n} {u : TmExpr n} {A A' : TyExpr n} → Derivation (Γ ⊢ u :> A) → (⊢ Γ == Δ) → Derivation (Γ ⊢ A) → Derivation (Γ ⊢ A') → Derivation (Γ ⊢ A == A') → Derivation (Δ ⊢ u :> A')
...
Subst1Tm : {Γ : Ctx n} {B : TyExpr n} {A : TyExpr (suc n)} {t : TmExpr (suc n)} {u : TmExpr n}
         → ⊢ Γ → Derivation ((Γ , B) ⊢ t :> A) → Derivation (Γ ⊢ u :> B)
         → Derivation (Γ ⊢ substTm t u :> substTy A u)
Subst1Tm dΓ dt du = SubstTm dt (idMorDerivation dΓ , congTmTy! ([idMor]Ty _) du)

Subst2Ty : {Γ : Ctx n} {B : TyExpr n} {C : TyExpr (suc n)} {D : TyExpr (suc (suc n))} {u v : TmExpr n}
         → ⊢ Γ → Derivation ((((Γ , B) , C)) ⊢ D) → Derivation (Γ ⊢ u :> B) → Derivation (Γ ⊢ v :> substTy C u) → Derivation (Γ ⊢ subst2Ty D u v)
Subst2Ty dΓ dA du dv = SubstTy dA ((idMorDerivation dΓ , congTmTy! ([idMor]Ty _) du) , dv)

Subst3Ty : {Γ : Ctx n} {B : TyExpr n} {C : TyExpr (suc n)} {D : TyExpr (suc (suc n))} {A : TyExpr (suc (suc (suc n)))} {u v w : TmExpr n}
         → ⊢ Γ → Derivation ((((Γ , B) , C) , D) ⊢ A) → Derivation (Γ ⊢ u :> B) → Derivation (Γ ⊢ v :> substTy C u) → Derivation (Γ ⊢ w :> subst2Ty D u v)
         → Derivation (Γ ⊢ subst3Ty A u v w)
Subst3Ty dΓ dA du dv dw = SubstTy dA (((idMorDerivation dΓ , congTmTy! ([idMor]Ty _) du) , dv) , dw)

Subst3TyEq : {Γ : Ctx n} {B : TyExpr n} {C : TyExpr (suc n)} {D : TyExpr (suc (suc n))} {A A' : TyExpr (suc (suc (suc n)))} {u u' v v' w w' : TmExpr n}
           → ⊢ Γ → Derivation ((((Γ , B) , C) , D) ⊢ A') → Derivation (Γ ⊢ u :> B) → Derivation (Γ ⊢ v :> substTy C u) → Derivation (Γ ⊢ w :> subst2Ty D u v)
           → Derivation ((((Γ , B) , C) , D) ⊢ A == A') → Derivation (Γ ⊢ u == u' :> B) → Derivation (Γ ⊢ v == v' :> substTy C u) → Derivation (Γ ⊢ w == w' :> subst2Ty D u v)
           → Derivation (Γ ⊢ subst3Ty A u v w == subst3Ty A' u' v' w')
Subst3TyEq dΓ dA' du dv dw dA= du= dv= dw= = SubstTyFullEq dA' (((idMorDerivation dΓ , congTmTy! ([idMor]Ty _) du) , dv) , dw) dA= (((MorRefl (idMorDerivation dΓ) , congTmEqTy! ([idMor]Ty _) du=) , dv=) , dw=)
...
-- CtxEqCtx1 {Γ = ◇} {Γ' = ◇} ttR = ttR
-- CtxEqCtx1 {Γ = Γ , A} {Γ' , A'} (dΓ= , dA , dA' , dA=) = (CtxEqCtx1 dΓ= , dA)

-- CtxEqCtx2 : {Γ Γ' : Ctx n} → ⊢ Γ == Γ' → ⊢ Γ'
-- CtxEqCtx2 {Γ = ◇} {Γ' = ◇} ttR = ttR
-- CtxEqCtx2 {Γ = Γ , A} {Γ' , A'} (dΓ= , dA , dA' , dA=) = (CtxEqCtx2 dΓ= , dA')



-- TyEqTy1 : {Γ : Ctx n} {A B : TyExpr n} → (⊢ Γ) → Derivation (Γ ⊢ A == B) → Derivation (Γ ⊢ A)
-- TyEqTy2 : {Γ : Ctx n} {A B : TyExpr n} → (⊢ Γ) → Derivation (Γ ⊢ A == B) → Derivation (Γ ⊢ B)
-- TmEqTm1 : {Γ : Ctx n} {u u' : TmExpr n} {A : TyExpr n} → (⊢ Γ) → Derivation (Γ ⊢ u == u' :> A) → Derivation (Γ ⊢ u :> A)
-- TmEqTm2 : {Γ : Ctx n} {u u' : TmExpr n} {A : TyExpr n} → (⊢ Γ) → Derivation (Γ ⊢ u == u' :> A) → Derivation (Γ ⊢ u' :> A)
...
-- TmEqTm1 dΓ (ConvEq dA du= dA=) = Conv dA (TyEqTy2 dΓ dA=) (TmEqTm1 dΓ du=) dA=
...
-- -- TmEqTm1 dΓ (BetaSig2 {B = B} dA dB da db) = Conv (SubstTy dB (idMorDerivation dΓ , congTm! ([idMor]Ty _) refl (Pr1 dA dB (Pair dA dB da db)))) (Pr2 dA dB (Pair dA dB da db)) (SubstTyMorEq dB (idMorDerivation dΓ , congTm! ([idMor]Ty _) refl (Pr1 dA dB (Pair dA dB da db))) (MorRefl (idMorDerivation dΓ) , congTmEqTy! ([idMor]Ty _) (BetaSig1 dA dB da db)))
...
-- TmEqTm2 dΓ (ConvEq dA du= dA=) = Conv dA (TyEqTy2 dΓ dA=) (TmEqTm2 dΓ du=) dA=
...
--   Conv (SubstTy (TyEqTy2 (dΓ , dA) dB=) (idMorDerivation dΓ , Conv dA (congTy (! ([idMor]Ty _)) dA) (TmEqTm2 dΓ da=) (congTyEq! refl ([idMor]Ty _) (TyRefl dA))))
--         (SubstTy (TyEqTy1 (dΓ , dA) dB=) (idMorDerivation dΓ , congTmTy! ([idMor]Ty _) (TmEqTm1 dΓ da=)))
--        (App (TyEqTy2 dΓ dA=)
--             (ConvTy (TyEqTy2 (dΓ , TyEqTy1 dΓ dA=) dB=) (CtxRefl dΓ , dA , TyEqTy2 dΓ dA= , dA= , dA=))
...
--                               (idMorDerivation dΓ , congTm! ([idMor]Ty _) refl (TmEqTm1 dΓ da=))
--                               dB=
--                               (MorRefl (idMorDerivation dΓ) , congTmEqTy! ([idMor]Ty _) da=)))
...
-- --              (Conv (SubstTy (TyEqTy1 (dΓ , dA) dB=) (idMorDerivation dΓ , congTm! ([idMor]Ty _) refl (TmEqTm1 dΓ da=)))
-- --                    (TmEqTm2 dΓ db=)
...
-- --                 (idMorDerivation dΓ , congTm! ([idMor]Ty _) refl (Conv (TyEqTy2 dΓ dA=)
-- --                                                                       (Pr1 (TyEqTy2 dΓ dA=)
-- --                                                                            (ConvTy (TyEqTy2 (dΓ , dA) dB=) (CtxRefl dΓ , dA , TyEqTy2 dΓ dA= , dA= , dA=))
-- --                                                                            (Conv (Sig dA (TyEqTy1 (dΓ , dA) dB=)) (TmEqTm2 dΓ du=) (SigCong dA dA= dB=)))
-- --                                                                       (TySymm dA=))))
...
-- --                                                                             (TySymm dA=)))
-- --                       (TySymm dB=)
-- --                       (MorRefl (idMorDerivation dΓ) , TmSymm (congTmEqTy! ([idMor]Ty _) (Pr1Cong dA dA= dB= du=))))
-- -- TmEqTm2 dΓ EmptyUUCong = EmptyUU
-- -- TmEqTm2 dΓ (EmptyelimCong dA= du=) = Conv (SubstTy (TyEqTy2 (dΓ , Empty) dA=) (idMorDerivation dΓ , TmEqTm2 dΓ du=)) (Emptyelim (TyEqTy2 (dΓ , Empty) dA=) (TmEqTm2 dΓ du=)) (SubstTyFullEq (TyEqTy1 (dΓ , Empty) dA=) (idMorDerivation dΓ , TmEqTm2 dΓ du=) (TySymm dA=) (MorRefl (idMorDerivation dΓ) , TmSymm du=))
-- -- TmEqTm2 dΓ UnitUUCong = UnitUU
-- -- TmEqTm2 dΓ TTCong = TT
-- -- TmEqTm2 dΓ (UnitelimCong dA= ddtt= du=) = Conv (SubstTy (TyEqTy2 (dΓ , Unit) dA=)
-- --                                                (idMorDerivation dΓ , TmEqTm2 dΓ du=))
-- --                                                (Unitelim (TyEqTy2 (dΓ , Unit) dA=) (Conv (SubstTy (TyEqTy1 (dΓ , Unit) dA=) (idMorDerivation dΓ , TT)) (TmEqTm2 dΓ ddtt=) (SubstTyEq dA= (idMorDerivation dΓ , TT))) (TmEqTm2 dΓ du=)) (SubstTyFullEq (TyEqTy1 (dΓ , Unit) dA=) (idMorDerivation dΓ , TmEqTm2 dΓ du=) (TySymm dA=) (MorRefl (idMorDerivation dΓ) , TmSymm du=))
...
-- --       ddO' = Conv (SubstTy dP (idMorDerivation dΓ , Zero)) (TmEqTm2 dΓ ddO=) (SubstTyEq dP= (idMorDerivation dΓ , Zero))
-- --       ddS' = ConvTm2' (TmEqTm2 ((dΓ , Nat) , dP) ddS=)
-- --                       (CtxRefl (dΓ , Nat) , dP , dP' , dP= , dP=)
-- --                       (SubstTy (WeakTy (WeakTy dP)) (idMorDerivation ((dΓ , Nat) , dP) , Suc (VarPrev Nat (VarLast Nat)))) 
-- --                       (SubstTyEq (WeakTyEq (WeakTyEq dP=)) (idMorDerivation ((dΓ , Nat) , dP) , Suc (VarPrev Nat (VarLast Nat)))) 
-- --       du' = TmEqTm2 dΓ du= in
-- --   Conv (SubstTy dP' (idMorDerivation dΓ , du'))
-- --        (Natelim dP' ddO' ddS' du')
-- --        (TySymm (SubstTyFullEq dP' (idMorDerivation dΓ , TmEqTm1 dΓ du=) dP= (MorRefl (idMorDerivation dΓ) , du=)))
...
-- TmEqTm2 dΓ (BetaPi dA dB du da) = SubstTm du (idMorDerivation dΓ , congTm! ([idMor]Ty _) refl da)
...
-- --   Conv (SubstTy (SubstTy (WeakTy (WeakTy dP)) (idMorDerivation ((dΓ , Nat) , dP) , Suc (VarPrev Nat (VarLast Nat)))) ((idMorDerivation dΓ , du) , Natelim dP ddO ddS du))
-- --        (SubstTm ddS ((idMorDerivation dΓ , du) , Natelim dP ddO ddS du))
-- --        (congTyRefl' (SubstTy dP (idMorDerivation dΓ , Suc du)) subst2Ty-substTy)  
...
-- MorEqMor1 {Δ = ◇} {δ = ◇} {◇} _ _ dδ= = ttR
-- MorEqMor1 {Δ = Δ , B} {δ = δ , u} {δ' , u'} dΓ (dΔ , _) (dδ= , du=) = (MorEqMor1 dΓ dΔ dδ= , TmEqTm1 dΓ du=)

-- MorEqMor2 {Δ = ◇} {δ = ◇} {◇} _ _ dδ= = ttR
-- MorEqMor2 {Δ = Δ , B} {δ = δ , u} {δ' , u'} dΓ (dΔ , dB) (dδ= , du=) = (MorEqMor2 dΓ dΔ dδ= , Conv (SubstTy dB (MorEqMor1 dΓ dΔ dδ=)) (SubstTy dB (MorEqMor2 dΓ dΔ dδ=)) (TmEqTm2 dΓ du=) (SubstTyMorEq dB (MorEqMor1 dΓ dΔ dδ=) dδ=))


-- MorSymm : {Γ : Ctx n} {Δ : Ctx m} {δ δ' : Mor n m} → ⊢ Γ → ⊢ Δ → Γ ⊢ δ == δ' ∷> Δ → Γ ⊢ δ' == δ ∷> Δ
-- MorSymm {Δ = ◇} {◇} {◇} _ _ ttR = ttR
...
-- {- added ⊢ Γ assumption, needed since Conv also needs derivabilty of the second type now -}
-- ConvTm2 : {Γ Δ : Ctx n} {u : TmExpr n} {A A' : TyExpr n} → Derivation (Γ ⊢ u :> A) → ⊢ Γ → (⊢ Γ == Δ) → Derivation (Γ ⊢ A == A') → Derivation (Δ ⊢ u :> A')
-- ConvTm2 du dΓ dΓ= dA= = Conv (ConvTy (TyEqTy1 (CtxEqCtx1 dΓ=) dA=) dΓ=) (ConvTy (TyEqTy2 dΓ dA=) dΓ=) (ConvTm du dΓ=) (ConvTyEq dA= dΓ=)

-- ConvTmEq2 : {Γ Δ : Ctx n} {u u' : TmExpr n} {A A' : TyExpr n} → Derivation (Γ ⊢ u == u' :> A) → (⊢ Γ == Δ) → Derivation (Γ ⊢ A == A') → Derivation (Δ ⊢ u == u' :> A')
...
-- SubstMorFullEq {Θ = ◇} {◇} {◇} dΓ dΔ ttR ttR dδ= = ttR
-- SubstMorFullEq {Θ = Θ , C} {θ , w} {θ' , w'} dΓ dΔ (dΘ , dC) (dθ= , dw=) dδ= =
--   (SubstMorFullEq dΓ dΔ dΘ dθ= dδ= , congTmEqTy ([]Ty-assoc _ _ _) (SubstTmFullEq (Conv (SubstTy dC (MorEqMor2 dΔ dΘ dθ=)) (SubstTy dC (MorEqMor1 dΔ dΘ dθ=)) (TmEqTm2 dΔ (ConvEq (SubstTy dC (MorEqMor1 dΔ dΘ dθ=)) dw= (SubstTyMorEq dC (MorEqMor1 dΔ dΘ dθ=) dθ=))) (SubstTyMorEq dC (MorEqMor2 dΔ dΘ dθ=) (MorSymm dΔ dΘ dθ=))) (MorEqMor1 dΓ dΔ dδ=) dw= dδ=))


-- SubstTyMorEq2 : {Γ : Ctx n} {Δ : Ctx m} {A A' : TyExpr m} {δ δ' : Mor n m}
--               → ⊢ Γ → ⊢ Δ → Derivation (Δ ⊢ A == A') → (Γ ⊢ δ == δ' ∷> Δ) → Derivation (Γ ⊢ A [ δ ]Ty == A' [ δ' ]Ty)
...
-- _,,_ : {Γ Γ' : Ctx n} {A A' : TyExpr n} → ⊢ Γ == Γ' → Derivation (Γ ⊢ A == A') → ⊢ (Γ , A) == (Γ' , A')
-- dΓ= ,, dA= =
--   let dΓ = CtxEqCtx1 dΓ=
--   in
