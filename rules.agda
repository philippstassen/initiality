...
-- SubstTm {u = tt} STARU dδ = TT
-- SubstTm {u = unitelim A dstarU u} (Unitelim dA ddstarU du) dδ = congTmTy! []Ty-substTy (Unitelim (SubstTy dA (WeakMor+ Unit dδ)) (congTmTy []Ty-substTy (SubstTm ddstarU dδ)) (SubstTm du dδ))
...
--                            (congTmTy []Ty-substTy (SubstTm ddO dδ))
--                            (congTmTy ([]Ty-substTy ∙ ap-substTy []Ty-weakenTy1-weakenTy1 refl) (SubstTm ddS (WeakMor+ dP (WeakMor+ Nat dδ)))))
-- SubstTmEq (BetaUnit dA ddtt) dδ = congTmEq! refl refl []Ty-substTy (BetaUnit (SubstTy dA (WeakMor+ Unit dδ)) (congTmTy []Ty-substTy (SubstTm ddstarU dδ)))
...
-- SubstTmMorEq STARU dδ dδ= = TTCong
-- SubstTmMorEq (Unitelim dA ddstarU du) dδ dδ= = congTmEqTy! []Ty-substTy (UnitelimCong (SubstTyMorEq dA (WeakMor+ Unit dδ) (WeakMor+Eq Unit dδ dδ=)) (congTmEqTy []Ty-substTy (SubstTmMorEq ddstarU dδ dδ=)) (SubstTmMorEq du dδ dδ=))
...
-- WeakTm STARU = TT
-- WeakTm (Unitelim dA ddstarU du) = congTmTy! weakenTy-substTy (Unitelim (WeakTy dA) (congTmTy weakenTy-substTy (WeakTm ddtt)) (WeakTm du))
-- WeakTm NatUU = NatUU
-- WeakTm Zero = Zero
...
SubstMorMorEq {Θ = ◇} {◇} starU dδ dδ= = starU
...
-- 
-- ConvTm2' : {Γ Δ : Ctx n} {u : TmExpr n} {A A' : TyExpr n} → Derivation (Γ ⊢ u :> A) → (⊢ Γ == Δ) → Derivation (Γ ⊢ A) → Derivation (Γ ⊢ A == A') → Derivation (Δ ⊢ u :> A')
...
-- ConvTm STARU dΓ= = TT
-- ConvTm (Unitelim dA ddstarU du) dΓ= = Unitelim (ConvTy dA (dΓ= , Unit , Unit , UnitCong , UnitCong)) (ConvTm ddstarU dΓ=) (ConvTm du dΓ=)
...
-- ConvTmEq (BetaUnit dA ddtt) dΓ= = BetaUnit (ConvTy dA (dΓ= , Unit , Unit , UnitCong , UnitCong)) (ConvTm ddstarU dΓ=)
...
-- -- TmEqTm1 dΓ (BetaUnit dA ddtt) = Unitelim dA ddstarUSTARU
...
--   Conv (SubstTy (TyEqTy2 (dΓ , dA) dB=) (idMorDerivation dΓ , Conv dA (TmEqTm2 dΓ da=) (congTyEq! refl ([idMor]Ty _) (TyRefl dA))))
--        (App (TyEqTy2 dΓ dA=)
--             (ConvTy (TyEqTy2 (dΓ , TyEqTy1 dΓ dA=) dB=) (CtxRefl dΓ , dA , TyEqTy2 dΓ dA= , dA= , dA=))
...
-- -- TmEqTm2 dΓSTARUCong =STARU
-- -- TmEqTm2 dΓ (UnitelimCong dA= ddtt= du=) = Conv (SubstTy (TyEqTy2 (dΓ , Unit) dA=)
-- --                                                (idMorDerivation dΓ , TmEqTm2 dΓ du=))
-- --                                                (Unitelim (TyEqTy2 (dΓ , Unit) dA=) (Conv (SubstTy (TyEqTy1 (dΓ , Unit) dA=) (idMorDerivation dΓ ,STARU)) (TmEqTm2 dΓ ddtt=) (SubstTyEq dA= (idMorDerivation dΓ ,STARU))) (TmEqTm2 dΓ du=)) (SubstTyFullEq (TyEqTy1 (dΓ , Unit) dA=) (idMorDerivation dΓ , TmEqTm2 dΓ du=) (TySymm dA=) (MorRefl (idMorDerivation dΓ) , TmSymm du=))
...
-- MorTran {Δ = ◇} {◇} {◇} {◇} _ _ starU starU =starU
...
-- ConvTm2 : {Γ Δ : Ctx n} {u : TmExpr n} {A A' : TyExpr n} → Derivation (Γ ⊢ u :> A) → (⊢ Γ == Δ) → Derivation (Γ ⊢ A == A') → Derivation (Δ ⊢ u :> A')
...
-- ConvMorEq {Δ = ◇} {Δ' = ◇} {δ = ◇} {◇} dδ= dΓ= dΔ= =starU
...
-- SubstMorFullEq {Θ = ◇} {◇} {◇} dΓ dΔ starU starU dδ= = starU
